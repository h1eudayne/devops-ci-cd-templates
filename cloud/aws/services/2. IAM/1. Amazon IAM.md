...
graph TD
    subgraph "Who (Ai)"
        who_user["IAM User / Group"]
        who_role["IAM Role / Service"]
    end

    subgraph "How (Cách thức truy cập)"
        how_api["AWS CLI / SDK"]
        how_console["AWS Console Login"]
    end

    subgraph "What (Tài nguyên truy cập)"
        what_s3["Amazon S3 Buckets"]
        what_ec2["Amazon EC2 Instances"]
        what_db["Amazon RDS Databases"]
    end

    who_user -->|Thông qua| how_api & how_console
    who_role -->|Thông qua| how_api
    how_api & how_console -->|Được phân quyền bởi IAM Policy| what_s3 & what_ec2 & what_db

    style who_user fill:#d4edda,stroke:#28a745,stroke-width:2px
    style who_role fill:#d4edda,stroke:#28a745,stroke-width:2px
    style how_api fill:#fff3cd,stroke:#ffc107,stroke-width:2px
    style how_console fill:#fff3cd,stroke:#ffc107,stroke-width:2px
    style what_s3 fill:#d1ecf1,stroke:#17a2b8,stroke-width:2px
    style what_ec2 fill:#d1ecf1,stroke:#17a2b8,stroke-width:2px
    style what_db fill:#d1ecf1,stroke:#17a2b8,stroke-width:2px
```
...
mindmap
  root((IAM Use Cases))
    Kiem soat truy cap dua tren thuoc tinh ABAC
      Ap dung quyen chi tiet
      Mo rong quy mo
      Dua vao phong ban job roles ten nhom
    Quan ly theo tai khoan va quy mo lon
      Quan ly tung tai khoan rieng le
      Mo rong quy mo tren nhieu tai khoan AWS
    Thiet lap quy tac bao ve phong ngua
      Dinh nghia ranh gioi bao mat
      Ngan chan thao tac nguy hiem
    Ap dung nguyen tac dac quyen toi thieu
      Thiet lap quyen
      Xac minh quyen
      Dieu chinh quy mo va tuy chinh
```
