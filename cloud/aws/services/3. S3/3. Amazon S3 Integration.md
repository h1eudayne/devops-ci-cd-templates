...
```mermaid
flowchart TD
    subgraph "Nguồn Ghi Dữ Liệu & Logs (Inputs)"
        vpc_alb_api["VPC / ALB / API Gateway"]
        iot["Thiết bị IoT"]
        backup["AWS Backup / File Backup"]
        cw_trail["CloudWatch / CloudTrail"]
    end

    subgraph "Lưu Trữ Trung Tâm"
        s3["Amazon S3 Bucket"]
    end

    subgraph "Ứng Dụng & Máy Chủ (Storage)"
        compute["ECS / EC2 / Lambda"]
    end

    subgraph "Xử Lý & Phân Phối Dữ Liệu (Outputs)"
        cf["Amazon CloudFront"]
        lambda["AWS Lambda"]
        glue["AWS Glue / Athena / Redshift"]
    end

    vpc_alb_api -->|Write Log| s3
    iot -->|Save File| s3
    backup -->|Backup File| s3
    cw_trail -->|Log Archive| s3
    
    compute <-->|File Storage| s3
    
    s3 -->|Static Web Hosting| cf
    s3 -->|Automation Process| lambda
    s3 -->|Data Source| glue

    style s3 fill:#e1f5fe,stroke:#0288d1,stroke-width:3px
    style compute fill:#efebe9,stroke:#5d4037,stroke-width:2px
    style vpc_alb_api fill:#fafafa,stroke:#616161,stroke-width:1px
    style iot fill:#fafafa,stroke:#616161,stroke-width:1px
    style backup fill:#fafafa,stroke:#616161,stroke-width:1px
    style cw_trail fill:#fafafa,stroke:#616161,stroke-width:1px
    style cf fill:#e8f5e9,stroke:#388e3c,stroke-width:2px
    style lambda fill:#e8f5e9,stroke:#388e3c,stroke-width:2px
    style glue fill:#e8f5e9,stroke:#388e3c,stroke-width:2px
```
